...
class MockedApiConnector: ApiConnectorProtocol {
	...
	func getPhotos(topicID: String?, page: Int, completion: @escaping ([TopPhotos.PhotoModel]?, Error?) -> Void) {
	...
	func getPhotoDetails(id: String, completion: @escaping (TopPhotos.PhotoDetailsModel?, Error?) -> Void) {
	...
	func getTopics(completion: @escaping ([TopPhotos.TopicModel]?, Error?) -> Void) {
